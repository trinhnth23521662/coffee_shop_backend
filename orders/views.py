...
class CreateOrderAPIView(APIView):
    ...
    def post(self, request):
        if not check_staff_permission(request):
            return Response({'status': 'error', 'message': 'Khong co quyen truy cap'}, status=403)
...
class OrderListAPIView(APIView):
    def get(self, request):
        try:
            ...
            # Query cơ bản - sử dụng field names như trong file đầu tiên
...
class OrderDetailAPIView(APIView):
    def get(self, request, ma_dh):
        try:
            ...
            return Response({'status': 'error', 'message': 'Khong tim thay don hang'}, status=404)
        except Exception as e:
            return Response({'status': 'error', 'message': str(e)}, status=500)
    ...
    def post(self, request):
        ...
        try:
            ...
            if not all([ma_dh, ma_mon]):
                return Response({'status': 'error', 'message': 'Thieu thong tin'}, status=400)
...
class UpdateOrderItemAPIView(APIView):
    ...
    def put(self, request, ma_ctdh):
        ...
        try:
            ...
            return Response({'status': 'error', 'message': 'Khong tim thay mon'}, status=404)
        except Exception as e:
            return Response({'status': 'error', 'message': str(e)}, status=500)
...
class ProcessPaymentAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not ma_dh:
                return Response({'status': 'error', 'message': 'Thieu ma_dh'}, status=400)
...
# ==================== API ONLINE CHO KHÁCH HÀNG ====================
@method_decorator(csrf_exempt, name='dispatch')
class CreateOnlineOrderAPIView(APIView):
    def post(self, request):
        if not request.body:
            return Response({"error": "Body JSON trống"}, status=400)
        ...
        try:
            ...
            if not ma_nd:
                return Response({"error": "Thiếu ma_nd"}, status=400)
    ...
    def post(self, request):
        try:
            ...
            if not ma_dh or not ma_sp:
                return Response({"error": "Thiếu mã đơn hoặc mã sản phẩm"}, status=400)

            try:
                don = DonHang.objects.get(ma_dh=ma_dh, nguon_don='online')
                sp = SanPham.objects.get(ma_sp=ma_sp)
            except DonHang.DoesNotExist:
                return Response({"error": "Đơn online không tồn tại"}, status=404)
...
class ViewOnlineOrderAPIView(APIView):
    def get(self, request):
        ma_dh = request.GET.get("ma_dh")

        if not ma_dh:
            return Response({"error": "Thiếu mã đơn"}, status=400)
...
class OnlineOrderHistoryAPIView(APIView):
    def get(self, request):
        ...
        try:
            ...
            return Response({"error": "Khách hàng online không tồn tại"}, status=404)
